...
fracpoly <- function(y=y, x=x, covar=NULL, family="gaussian"){
  ...
  if(is.null(covar)){missing <- is.na(y) | is.na(x)}else{missing <- is.na(y) | is.na(x) | apply(is.na(covar),2,sum)>0}
  y <- y[!missing]
  x <- x[!missing]
...
mvshape <- function(y=y, x=x, covar=NULL, study=NULL, ngrp=10, refgrp=1, family="gaussian", float=FALSE, method="reml"){
  ...
  # Remove missing values
  if(length(covar)==0){missing <- is.na(y) | is.na(x)}else{missing <- is.na(y) | is.na(x) | apply(is.na(covar),2,sum)>0}
  y <- y[!missing]
  x <- x[!missing]
  if(!is.null(covar)){covar_names <- names(covar); covar <- as.data.frame(covar[!missing,]); names(covar) <- covar_names}
